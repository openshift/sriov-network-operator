...
func (p *IntelPlugin) OnNodeStateChange(old, new *sriovnetworkv1.SriovNetworkNodeState) (needDrain bool, needReboot bool, err error) {
	...
	if needVfioDriver(new) {
		...
		if needReboot, err = tryEnableIommuInKernelArgs(); err != nil {
			glog.Errorf("intel-plugin OnNodeStateAdd():fail to enable iommu in kernel args: %v", err)
			return
		}
	}
	if needReboot {
...
func (p *IntelPlugin) Apply() error {
	...
	if p.LoadVfioDriver == loading {
		if err := utils.LoadKernelModule("vfio_pci"); err != nil {
			glog.Errorf("intel-plugin Apply(): fail to load vfio_pci kmod: %v", err)
			return err
		}
...
func tryEnableIommuInKernelArgs() (bool, error) {
	glog.Info("intel-plugin tryEnableIommuInKernelArgs()")
	...
	if err := cmd.Run(); err != nil {
		glog.Errorf("intel-plugin tryEnableIommuInKernelArgs(): fail to enable iommu %s: %v", args, err)
		return false, err
	}
	...
	if err == nil {
		if i > 0 {
			glog.Infof("intel-plugin tryEnableIommuInKernelArgs(): need to reboot node")
			return true, nil
		}
	}
	return false, err
